...
public partial class FlomaContext : DbContext
{
    public FlomaContext()
    {
    }

    public FlomaContext(DbContextOptions<FlomaContext> options)
        : base(options)
    {
    }
    ...
    protected override void OnConfiguring(DbContextOptionsBuilder optionsBuilder)
    {
        optionsBuilder.UseMySql("Server=localhost;Database=floma;User=root;Password=floma321;",
            new MySqlServerVersion(new Version(8, 0, 21)))
                .UseLazyLoadingProxies(); // Habilita la carga diferida a través de proxies
